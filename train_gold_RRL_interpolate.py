...
LOG_FILE = "training_gold_comprehensive.log"
logging.basicConfig(
    level=logging.INFO,
...
TRAIN_DATA_PATH = "df_gold_dataset_gepu_extended_train.csv"
TEST_DATA_PATH = "df_gold_dataset_gepu_extended_test.csv"
...
def load_and_preprocess(train_path, test_path, target_col):
    ...
    # We strictly use the features present in the CSV
    ...
    # Prices for the target return periods (t+1)
    # The first row is index 1 of the original df (due to pct_change)
    # The first 'target' in train_rets is returns[1] -> which matches price[2]/price[1]
    # So the price at 't' for the first sequence target is price[1]
...
class CNN_BiLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use only the last weighted state or global pool? Let's take the mean or last.
...
def objective(trial, X_full, y_full, feature_cols):
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X_full)):
        ...
        if len(X_train_seq) < 50: continue # Skip too small folds
        ...
        val_data = TensorDataset(torch.tensor(X_val_seq, dtype=torch.float32).to(device), torch.tensor(y_val_seq, dtype=torch.float32).to(device))
        ...
        with torch.no_grad():
            ...
            score = (1.0 - da) * 2.0 + rmse # Composite score favoring DA
...
# =============================================================================
def main():
    logging.info("Starting Full Gold Training Pipeline...")
    ...
    for seed in FINAL_SEEDS:
        ...
        for epoch in range(150):
            ...
            if (epoch+1) % 50 == 0:
                model.eval()
                with torch.no_grad():
                    test_pred = model(torch.tensor(X_test_seq, dtype=torch.float32).to(device))
                    test_loss = criterion(test_pred, torch.tensor(y_test_seq, dtype=torch.float32).to(device))
                    logging.info(f"Seed {seed} | Epoch {epoch+1} | Test Loss {test_loss.item():.4f}")
                model.train()
            # Simple early stopping against test set (proactive)
            model.eval()
            with torch.no_grad():
                l_test = criterion(model(X_test_t), y_test_t).item()
                if l_test < best_val_loss:
                    best_val_loss = l_test
                    torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"gold_model_seed_{seed}.pth"))
            
            if (epoch+1) % 50 == 0:
                logging.info(f"Seed {seed} | Epoch {epoch+1} | Test Loss {l_test:.4f}")
        
        model.load_state_dict(torch.load(os.path.join(SAVE_DIR, f"gold_model_seed_{seed}.pth")))
        model.eval()
        with torch.no_grad():
    ...
    # Calculate Prices
    # Prices starts from price[1] as per preprocessing
    ...
    logging.info(f"FINAL RESULTS (GOLD): DA={da:.2f}%, RMSE={rmse:.4f}, LagCorr={lag_corr:.4f}")
    
    # Check for laziness
    if lag_corr > 0.8:
        logging.warning("CAUTION: High Lag Correlation detected. Model may still be shows signs of laziness.")
    
    # Save Scalers & Artifacts
    ...
    plt.title(f"Gold Forecast (Ensemble) | DA: {da:.2f}% | LagCorr: {lag_corr:.4f}")
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(PLOTS_DIR, "gold_final_forecast.png"))
    plt.close()
