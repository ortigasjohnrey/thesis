...
def simulate_adaptive_pro(asset_name, config):
    print(f"\n>>> Starting PRO ADAPTIVE Simulation for {asset_name.upper()}...")
    ...
    for i in tqdm(range(len(test_df)), desc=f"{asset_name} Pro-Learning"):
        # 1. PREDICT TODAY (Using Adaptive Weights)
        ...
        # Calculate dynamic weights from recent errors
        inv_errors = []
        ...
        weighted_scaled_pred = np.average(seed_preds, weights=weights)
        
        pred_return = y_scaler.inverse_transform(np.array([[weighted_scaled_pred]])).item()
        ...
        # 3. UPDATE WEIGHTS (Evaluate seed predictions against reality)
        for s_idx in range(len(models)):
            s_pred_scaled = seed_preds[s_idx]
            s_pred_ret = y_scaler.inverse_transform(np.array([[s_pred_scaled]])).item()
            err = abs(actual_return - s_pred_ret)
            seed_perf[s_idx].append(err)
            if len(seed_perf[s_idx]) > 10: seed_perf[s_idx].pop(0)

        # 4. WINDOWED RETRAINING (Train on last 5 days)
        # Re-build multi-sample batch from recent context
        temp_df = pd.concat([context_df, test_df.iloc[[i]]], ignore_index=True)
        num_full = temp_df[features].tail(lookback + window_size + 1).copy()
        for c in features: num_full[c] = pd.to_numeric(num_full[c], errors="coerce")
        ret_full = num_full.pct_change().dropna()
        
        if len(ret_full) >= lookback + 1:
            X_batch_list, y_batch_list = [], []
            for b_idx in range(min(window_size, len(ret_full)-lookback)):
                s = len(ret_full) - lookback - b_idx - 1
                e = len(ret_full) - b_idx - 1
                X_batch_list.append(x_scaler.transform(ret_full[features].iloc[s:e]))
                y_batch_list.append(y_scaler.transform(np.array([[ret_full[target_col].iloc[e]]])))

            X_b = torch.tensor(np.array(X_batch_list), dtype=torch.float32).to(device)
            y_b = torch.tensor(np.array(y_batch_list), dtype=torch.float32).squeeze(1).to(device)
            
            for m in models:
                m.train()
                opt = torch.optim.Adam(m.parameters(), lr=float(params["lr"])*0.4)
                for _ in range(2):
                    opt.zero_grad()
    ...
    print(f"\n--- {asset_name.upper()} ADAPTIVE PRO RESULTS ---")
    print(f"  DA:   {da:.2%}")
...
if __name__ == "__main__":
    ...
    for asset, cfg in ASSET_CONFIG.items():
        _, stats = simulate_adaptive_pro(asset, cfg)
        summaries[asset] = stats
    print("\n\nPRO ADAPTIVE COMPARISON SUMMARY:")
