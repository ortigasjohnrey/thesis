...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("training_silver_rrl.log"),
...
# 2) User Inputs
# =========================
CLEAN_DF_PATH = "silver_RRL_interpolate_train.csv"
...
SAVE_ROOT_DIR = "models/silver_RRL_interpolate"
...
for seed in FINAL_SEEDS:
    ...
    plt.legend()
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(PLOTS_DIR, f"silver_yahoo_seed_{seed}_loss.png"))
    plt.close()
    ...
    plt.plot(y_pred_test, label="Predicted Price", linestyle="--", color='red')
    plt.title(f"Seed {seed} - Walkforward Test | RMSE=${rmse:.2f}, R²={r2:.4f}")
    plt.xlabel("Test Sequence Index")
    plt.ylabel("Price ($)")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(PLOTS_DIR, f"silver_yahoo_seed_{seed}_predictions.png"))
    plt.close()
